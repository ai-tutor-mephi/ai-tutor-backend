...
- **Описание:** Загружает файлы и создает на их основе новый диалог. Если передан JSON part `request.title`, название диалога берется из него. Если `title` отсутствует или пустой, название совпадает с названием первого загруженного файла.
- **Тип запроса:** `multipart/form-data`
- **Тело запроса:** Поле с ключом files и типом File. Опционально JSON part с ключом `request`:
    ```json
    {
